...
public class StringUtil {
    ...
    public static String trimSubEnd(String str) {
        return trimSubEnd(str, "/", 1);
    }

    public static String trimSubEnd2(String str) {
        return trimSubEnd(str, "/", 2);
    }

    public static String trimSubEnd(String str, String prefix, Integer num) {
        String result = "";
        String[] strs = str.split(prefix);
        for (int i = 0; i < strs.length - num; i++) {
            if (StringUtils.isBlank(strs[i])) continue;
            result += prefix + strs[i];
        }
        return trimStart(result, prefix);
    }
